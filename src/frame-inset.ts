...
function detectScoreBased(
  ...
  for (let y = 0; y < height; y++) {
    ...
    rowScore[y] = opaque >= MIN_OPAQUE ? white / opaque : 0;
  }
  ...
  for (let x = 0; x < width; x++) {
    ...
    colScore[x] = opaque >= MIN_OPAQUE ? white / opaque : 0;
  }
...
function detectInnerRun(
  ...
  const rowRuns: Array<{ start: number; end: number } | null> = new Array(
    height,
  ).fill(null);
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x <= width; x++) {
      const light =
        x < width ? isLightB(data, (y * width + x) * 4) : false;
      if (light) {
        if (start === -1) start = x;
      } else {
        if (start !== -1) {
