...
async function toCanvas(
   ...
   if (source instanceof HTMLCanvasElement) {
      if (!maxSize || (source.width <= maxSize && source.height <= maxSize))
         return source;
   }
...
function isInCenter(
   ...
   const cx0 = w * (0.5 - fraction / 2),
      cx1 = w * (0.5 + fraction / 2);
   const cy0 = h * (0.5 - fraction / 2),
      cy1 = h * (0.5 + fraction / 2);
   ...
   for (let py = 0; py < h; py += step) {
      for (let px = 0; px < w; px += step) {
         ...
         if (data[i + 3] < 128) {
            sumR += 255;
            sumG += 255;
            sumB += 255; // transparent → treat as white
         } else {
            sumR += data[i];
            sumG += data[i + 1];
            sumB += data[i + 2];
...
/**
 * Detects the bounding box of the QR cutout area by finding the largest
 * contiguous rectangular hole (transparent or "cutout-colored") in the frame.
 */
function detectCutoutBounds(
   ...
   // Fallback: insufficient contrast — return centered 50%
   if (Math.abs(cutoutLum - frameLum) < 0.06) {
      return {
         x: Math.round(w * 0.25),
         y: Math.round(h * 0.25),
   ...
   const isCutout = (i: number) => {
      if (data[i + 3] < 128) return true; // transparent is cutout
      const lum = luminance(data[i], data[i + 1], data[i + 2]);
      return Math.abs(lum - cutoutLum) < Math.abs(lum - frameLum);
   };

   // Histogram-based largest rectangle detection
   ...
   for (let py = 0; py < h; py += step) {
      ...
      // Largest rectangle in histogram (O(w) algorithm)
      const stack: number[] = [];
      for (let i = 0; i <= w; i += step) {
         const curH = i < w ? colHeights[i] : 0;
         while (stack.length > 0 && curH < colHeights[stack[stack.length - 1]]) {
            const height = colHeights[stack.pop()!];
            const left = stack.length > 0 ? stack[stack.length - 1] + step : 0;
            const width = i - left;
            const area = width * height;
            if (area > best.area) {
               best = { x: left, y: py - height + step, width, height, area };
            }
         }
         if (i < w) stack.push(i);
      }
   ...
   return {
      x: Math.max(0, best.x),
      y: Math.max(0, best.y),
      width: Math.max(1, Math.min(w - best.x, best.width)),
      height: Math.max(1, Math.min(h - best.y, best.height)),
   };
}
...
function buildQROptions(palette: string[], cutoutColor: string): Partial<Options> {
...
function canvasToBlob(
   ...
   return new Promise((resolve, reject) => {
      canvas.toBlob(
         blob =>
            blob ? resolve(blob) : reject(new Error('compressFrame: toBlob failed')),
         mime,
         quality,
      );
   });
